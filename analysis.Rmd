...
cancer_data_wasr <- cancer_data_tidy %>% 
...
hb_shapes@data <- hb_shapes@data %>% 
  inner_join(cancer_data_wasr, by = c("HBCode" = "hb"))
```

```{r}
hb_shapes@data$id <- rownames(hb_shapes@data)
...
```{r}
hb_shapes_points <- fortify(hb_shapes, region = "id")
```

```{r}
hb_shapes_df = inner_join(hb_shapes_points, hb_shapes@data, by = "id")
```
...
ggplotly(
  hb_shapes_df %>% 
    ggplot() +
      aes(long, lat, group = `group.x`, fill = -value,
          text = paste(HBName, "\n", round(value, 2), sep = "")) +
